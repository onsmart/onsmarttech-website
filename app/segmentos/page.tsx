"use client"

import type React from "react"
import Link from "next/link"
import Image from "next/image"
import { useState } from "react"
import { ArrowLeft, Target, Building2, Factory, ShoppingCart, GraduationCap, Heart, Briefcase, Code, Settings, Users, BarChart, Zap, Globe, Layers, Phone, Mail, MapPin, ChevronDown, ChevronRight, Menu, X } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import Header from "@/components/header"

export default function SegmentosPage() {
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null)
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  
  const segmentos = [
    {
      title: "Indústria",
      description: "Otimização de processos produtivos e gestão de ativos com tecnologia IoT e automação.",
      icon: <Factory className="size-8" />,
      benefits: [
        "Automação Industrial",
        "Monitoramento IoT",
        "Gestão de Ativos",
        "Segurança de Dados",
      ],
    },
    {
      title: "Varejo",
      description: "Soluções para melhorar a experiência do cliente, otimizar vendas e gerenciar estoque.",
      icon: <ShoppingCart className="size-8" />,
      benefits: [
        "E-commerce",
        "PDV Inteligente",
        "CRM",
        "Análise de Vendas",
      ],
    },
    {
      title: "Saúde",
      description: "Tecnologias para hospitais e clínicas, focando em gestão de pacientes e telemedicina.",
      icon: <Heart className="size-8" />,
      benefits: [
        "Prontuário Eletrônico",
        "Telemedicina",
        "Gestão Hospitalar",
        "Segurança de Dados",
      ],
    },
    {
      title: "Educação",
      description: "Plataformas de ensino a distância, gestão acadêmica e ambientes de aprendizado interativos.",
      icon: <GraduationCap className="size-8" />,
      benefits: [
        "EAD Personalizado",
        "Gestão Acadêmica",
        "Ambientes Virtuais",
        "Colaboração Online",
      ],
    },
    {
      title: "Serviços",
      description: "Ferramentas para otimizar a prestação de serviços, gestão de equipes e relacionamento com clientes.",
      icon: <Briefcase className="size-8" />,
      benefits: [
        "Gestão de Projetos",
        "CRM de Serviços",
        "Automação de Tarefas",
        "Suporte ao Cliente",
      ],
    },
    {
      title: "Tecnologia",
      description: "Soluções avançadas para empresas de tecnologia, incluindo desenvolvimento e infraestrutura.",
      icon: <Code className="size-8" />,
      benefits: [
        "DevOps",
        "Cloud Computing",
        "Cibersegurança",
        "Big Data",
      ],
    },
  ]

  return (
    <div className="flex min-h-[100dvh] flex-col bg-background">
      <Header />

      <main className="flex-1">
        {/* Hero Section */}
        <section className="w-full py-20 md:py-32 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-background via-muted/20 to-background"></div>
          <div className="absolute -top-24 -left-24 w-96 h-96 bg-primary/20 rounded-full blur-3xl"></div>
          <div className="absolute -bottom-24 -right-24 w-96 h-96 bg-primary/10 rounded-full blur-3xl"></div>
          
          <div className="container px-4 md:px-6 relative z-10">
            <div className="max-w-4xl mx-auto text-center">
              <div className="mb-8">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary border border-primary/20 text-sm font-medium mb-6">
                  <Target className="size-4" />
                  Especialização por Segmento
                </div>
              </div>
              
              <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-8">
                Segmentos de <span className="text-primary">Atuação</span>
              </h1>
              <div className="w-24 h-1 bg-primary mx-auto mb-8"></div>
              <p className="text-xl text-muted-foreground leading-relaxed mb-8">
                Soluções tecnológicas especializadas para diferentes segmentos de mercado, 
                desenvolvidas para atender às necessidades específicas de cada área de negócio.
              </p>
              
              {/* Estatísticas */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12">
                {[
                  { number: "6+", label: "Segmentos Atendidos" },
                  { number: "50+", label: "Soluções Específicas" },
                  { number: "200+", label: "Projetos Entregues" },
                  { number: "98%", label: "Satisfação" }
                ].map((stat, i) => (
                  <div key={i} className="text-center">
                    <div className="text-3xl md:text-4xl font-bold text-primary mb-2">
                      {stat.number}
                    </div>
                    <div className="text-sm text-muted-foreground">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* Segmentos Grid */}
        <section className="w-full py-20 md:py-32 bg-muted/30">
          <div className="container px-4 md:px-6">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-6">
                Nossos Segmentos de Especialização
              </h2>
              <div className="w-16 h-1 bg-primary mx-auto mb-8"></div>
              <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                Cada segmento possui soluções específicas desenvolvidas com base em anos de experiência 
                e conhecimento profundo das necessidades de cada mercado.
              </p>
            </div>

            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {segmentos.map((segmento, i) => (
                <Card key={i} className="h-full group hover:shadow-xl transition-all duration-300 border-border/40 bg-background hover:border-primary/20">
                  <CardContent className="p-8">
                    <div className="size-20 rounded-2xl bg-primary/10 flex items-center justify-center text-primary mb-6 group-hover:bg-primary group-hover:text-primary-foreground transition-colors duration-300">
                      {segmento.icon}
                    </div>
                    
                    <h3 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors">
                      {segmento.title}
                    </h3>
                    
                    <p className="text-muted-foreground leading-relaxed mb-6">
                      {segmento.description}
                    </p>
                    
                    <div className="space-y-3">
                      <h4 className="text-sm font-semibold text-primary mb-4 flex items-center gap-2">
                        <div className="size-2 bg-primary rounded-full"></div>
                        Principais Soluções
                      </h4>
                      <div className="grid grid-cols-2 gap-2">
                        {segmento.benefits.map((benefit, j) => (
                          <div
                            key={j}
                            className="text-sm text-muted-foreground flex items-center gap-2 p-2 rounded-lg bg-muted/50 group-hover:bg-primary/10 transition-colors"
                          >
                            <div className="size-1.5 bg-primary rounded-full flex-shrink-0"></div>
                            <span className="text-xs">{benefit}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="w-full py-20 md:py-32 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-background via-muted/20 to-background"></div>
          <div className="absolute -top-24 -right-24 w-96 h-96 bg-primary/20 rounded-full blur-3xl"></div>
          <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-primary/10 rounded-full blur-3xl"></div>
          
          <div className="container px-4 md:px-6 relative z-10">
            <div className="max-w-4xl mx-auto text-center">
              <div className="mb-8">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary border border-primary/20 text-sm font-medium mb-6">
                  <Target className="size-4" />
                  Solução Personalizada
                </div>
              </div>
              
              <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-8">
                Não Encontrou Seu <span className="text-primary">Segmento?</span>
              </h2>
              <div className="w-24 h-1 bg-primary mx-auto mb-8"></div>
              <p className="text-xl text-muted-foreground leading-relaxed mb-12 max-w-3xl mx-auto">
                Desenvolvemos soluções personalizadas para qualquer segmento de mercado. 
                Entre em contato conosco e descubra como podemos ajudar seu negócio a crescer.
              </p>
              
              <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-8 mb-8">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-8">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <div className="size-8 rounded-full bg-primary/10 flex items-center justify-center">
                      <Target className="size-4 text-primary" />
                    </div>
                    <span>Análise Personalizada</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <div className="size-8 rounded-full bg-primary/10 flex items-center justify-center">
                      <Building2 className="size-4 text-primary" />
                    </div>
                    <span>Desenvolvimento Sob Medida</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <div className="size-8 rounded-full bg-primary/10 flex items-center justify-center">
                      <Factory className="size-4 text-primary" />
                    </div>
                    <span>Suporte Especializado</span>
                  </div>
                </div>
                
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button size="lg" className="rounded-full h-14 px-8 text-base bg-primary hover:bg-primary/90">
                    Falar com Especialista
                    <ArrowLeft className="ml-2 size-4 rotate-180" />
                  </Button>
                  <Button
                    size="lg"
                    variant="outline"
                    className="rounded-full h-14 px-8 text-base border-primary text-primary hover:bg-primary hover:text-primary-foreground"
                  >
                    Solicitar Orçamento
                  </Button>
                </div>
              </div>
              
              <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                <Target className="size-4" />
                <span>Consultoria gratuita de 30 minutos para novos segmentos!</span>
              </div>
            </div>
          </div>
        </section>
      </main>

      {/* Footer */}
      <footer className="w-full border-t backdrop-blur-sm" style={{ backgroundColor: 'hsl(var(--footer-bg))' }}>
        <div className="container flex flex-col gap-8 px-4 py-16 md:px-6">
          <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-4">
            <div className="space-y-4">
              <div className="flex items-center gap-2 font-bold text-xl">
                <Image
                  src="/logo.png"
                  alt="OnSmart Tech Logo"
                  width={120}
                  height={40}
                  className="object-contain hover:scale-105 transition-transform duration-300"
                  style={{ maxWidth: '120px', height: 'auto' }}
                />
              </div>
              <p className="text-sm text-white/80 leading-relaxed">
                Transformando negócios através da tecnologia. Soluções digitais inovadoras para acelerar seu
                crescimento.
              </p>
            </div>
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-white">Contato</h4>
              <ul className="space-y-2 text-sm">
                <li className="flex items-center gap-2 text-white/80">
                  <Phone className="size-4" />
                  <span>(11) 5093-1836</span>
                </li>
                <li className="flex items-center gap-2 text-white/80">
                  <Mail className="size-4" />
                  <span>contato@onsmart.com.br</span>
                </li>
                <li className="flex items-center gap-2 text-white/80">
                  <MapPin className="size-4" />
                  <span>São Paulo, SP</span>
                </li>
              </ul>
            </div>
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-white">Soluções</h4>
              <ul className="space-y-2 text-sm">
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Desenvolvimento Web
                  </Link>
                </li>
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Apps Mobile
                  </Link>
                </li>
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Consultoria Digital
                  </Link>
                </li>
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Cloud Computing
                  </Link>
                </li>
              </ul>
            </div>
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-white">Empresa</h4>
              <ul className="space-y-2 text-sm">
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Carreiras
                  </Link>
                </li>
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Blog
                  </Link>
                </li>
                <li>
                  <Link href="#" className="text-white/80 hover:text-white transition-colors">
                    Suporte
                  </Link>
                </li>
              </ul>
            </div>
          </div>
          <div className="flex flex-col gap-4 sm:flex-row justify-between items-center border-t border-white/20 pt-8">
            <p className="text-xs text-white/80">
              &copy; {new Date().getFullYear()} OnSmart Tech Solutions. Todos os direitos reservados.
            </p>
            <div className="flex gap-4">
              <Link href="#" className="text-xs text-white/80 hover:text-white transition-colors">
                Política de Privacidade
              </Link>
              <Link href="#" className="text-xs text-white/80 hover:text-white transition-colors">
                Termos de Uso
              </Link>
            </div>
          </div>
        </div>
      </footer>
    </div>
  )
}